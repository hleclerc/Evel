...
public:
    ...
    CbString( IKnowWhatIDo, SharedBuffer *buff, PT off, PT len ); ///< dangerous (because if you have buff, you can make weird thing. Normally, buff should be managed by CbQueue)
    ...
    PT   nbuf () const { PT res = 0; for( const SharedBuffer *b = beg; b; b = b->next ) ++res; return res; }
    ...
    void data_visitor( std::function<void(const PI8 *,const PI8 *)> func ) const { ///< op must return true to continue to read
        visitor( [ &func ]( const SharedBuffer *b, ST bb, ST eb ) {
    ...
    bool data_visitor( std::function<bool(const PI8 *,const PI8 *)> func ) const { ///< op must return true to continue to read
        return visitor( [ &func ]( const SharedBuffer *b, ST bb, ST eb ) {
...
protected:
    ...
    bool visitor( const Op &op ) const { ///< op must return true to continue to read
        if ( off < end ) {
            ...
            // use first buff
            const SharedBuffer *b = beg, *n = b->next; // `n` and `e` must be computed now
    ...
    static PI8 read_byte_wo_dec_ref( const SharedBuffer *&beg, PT &off, PT &end ) {
        PI8 res = beg->data[ off++ ];
        if ( off == beg->used ) {
            end -= beg->used; // != 0 because we know that end > off (previous test)
            beg = beg->next;
