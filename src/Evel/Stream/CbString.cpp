...
CbString::CbString( const CbString &bs, PT b_off, PT b_len ) : beg( 0 ), off( 0 ), end( 0 ) {
    if ( not b_len )
        return;
    bs.visitor( [ this, &b_off, b_len ]( const SharedBuffer *b, PT bd, PT ed ) {
        PT sd = ed - bd;
...
CbString::CbString( const CbString &bs ) : beg( bs.beg ), off( bs.off ), end( bs.end ) {
    bs.visitor( []( const SharedBuffer *b, PT, PT ) {
        inc_ref( b );
        return true;
    } );
}
...
CbString::CbString( const CbQueue &bs, PT s_off, PT s_len ) : beg( bs.beg ), off( bs.off + s_off ), end( bs.off + std::min( bs.size(), s_off + s_len ) ) {
    ...
    visitor( []( const SharedBuffer *b, PT, PT ) {
        inc_ref( b );
        return true;
    } );
}
...
CbString::CbString( IKnowWhatIDo, SharedBuffer *buff, PT off, PT len ) : beg( buff ), off( off ), end( off + len ) {
...
CbString::CbString( const std::string &bs ) {
    SharedBuffer *buf = SharedBuffer::New( bs.size() );
...
bool CbString::starts_with( const void *data, PT size ) {
    visitor( [ &data, &size ]( const SharedBuffer *b, unsigned off, unsigned used ) {
...
CbString CbString::read_line( char sep, bool skip_void_lines ) {
    ...
    // find sep
    PT pcr = 0;
    visitor( [ &pcr, sep ]( const SharedBuffer *b, unsigned off, unsigned used ) {
...
void CbString::write_to_stream( std::ostream &os ) const {
    int cpt = 0;
    visitor( [ &os, &cpt ]( const SharedBuffer *b, PT beg, PT end ) {
