...
CbQueue::CbQueue() : beg( 0 ), end( (SharedBuffer *)8 /*because error() have to return false*/ ), off( 0 ) {
}

CbQueue::~CbQueue() {
    for( const SharedBuffer *b = beg; b; ) {
        const SharedBuffer *o = b;
        b = b->next;
        dec_ref( o );
    }
}
...
void CbQueue::clear() {
    ...
    for( SharedBuffer *b = beg; b; b = b->next ) {
        if ( b->cpt_use == 0 ) {
            SharedBuffer *n = b->next;
            b->used = 0;
            b->next = 0;
            beg = b;
            end = b;
            off = 0;
            for( SharedBuffer *o; n; n = o ) {
                o = n->next;
                dec_ref( n );
            }
            return;
...
void CbQueue::free() {
    ...
    for( const SharedBuffer *b = beg, *n; b; b = n ) {
        n = b->next;
        dec_ref( b );
    }
...
CbQueue::operator std::string() const {
    ...
    visitor( [ &res ]( const SharedBuffer *b, PT bl, PT el ) {
...
void CbQueue::write_some( const void *data, PT size ) {
    ...
    if ( beg ) {
        ...
        end = SharedBuffer::New( std::max( PT( SharedBuffer::default_size ), size ), end );
    } else {
        beg = SharedBuffer::New( std::max( PT( SharedBuffer::default_size ), size ) );
        end = beg;
        off = 0;
    }
    ...
    while ( size > end->size ) {
        ...
        end = SharedBuffer::New( SharedBuffer::default_size, end );
    }
...
void CbQueue::add_buff( IKnowWhatIDo, SharedBuffer *buff, unsigned offset_in_buff ) {
...
void CbQueue::insert_some( PT pos, const void *data, PT size ) {
    ...
    SharedBuffer *b = SharedBuffer::New( size );
...
void *CbQueue::ptr( PT offset ) {
    ...
    // next buffers
    for( SharedBuffer *buf = beg->next; buf; buf = buf->next ) {
        if ( offset < buf->used )
...
void CbQueue::write_to_stream( std::ostream &os ) const {
    int cpt = 0;
    visitor( [ & ]( const SharedBuffer *b, PT beg, PT end ) {
