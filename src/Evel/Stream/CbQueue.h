...
public:
    ...
    PT   size () const { PT res = 0; for( SharedBuffer *b = beg; b; b = b->next ) res += b->used; return res - off; }
    PT   nbuf () const { PT res = 0; for( SharedBuffer *b = beg; b; b = b->next ) ++res; return res; }
    ...
    void write_byte( PI8 val ) {
        if ( not beg ) init_beg(); else if ( not end->room() ) end = SharedBuffer::New( SharedBuffer::default_size, end );
        end->data[ end->used++ ] = val;
    }

    void write_byte_wo_beg_test( PI8 val ) { ///< can be used after a first write( len != 0 ) or a first write_byte
        if ( not end->room() ) end = SharedBuffer::New( SharedBuffer::default_size, end );
        end->data[ end->used++ ] = val;
    }

    void add_buff( IKnowWhatIDo, SharedBuffer *buff, unsigned offset_in_buff = 0 ); ///< after that, buff should not be reused elsewhere
    ...
    void new_buff( PT size = SharedBuffer::default_size ) { if ( not beg ) init_beg(); else end = SharedBuffer::New( size, end ); } ///< should not be called manually (expected for test purpose)
    ...
    bool ack_read_some( ST len ) { ///< return true if ok to `len` bytes. Else, set end to 0 (to signal an error) and return false.
        len += off;
        for( SharedBuffer *b = beg; b; b = b->next ) {
            len -= b->used;
            if ( len <= 0 )
                return true;
        }
    ...
    void data_visitor( std::function<void(const PI8 *,const PI8 *)> func ) const { ///< op must return true to continue to read
        visitor( [ &func ]( const SharedBuffer *b, PT bb, PT eb ) {
    ...
    void remove_chunks( const TV0 &positions, const TV1 &sizes ) {
        SharedBuffer  *buf_writer = beg;
        unsigned off_writer = off;
        PT       acc_pos = 0;
        PT       cur_pos = 0;
        PT       avoid   = 0;
        visitor( [ & ]( const SharedBuffer *b, PT bd, PT ed ) { // buf_writer, &off_writer, &acc_pos, &cur_pos, &avoid, &positions, &sizes
            // TODO: optimize
        ...
        for( const SharedBuffer *n = buf_writer->next; n; ) {
            const SharedBuffer *o = n;
            n = n->next;
            dec_ref( o );
        }
...
protected:
    ...
    bool visitor( const Op &op ) const { ///< op must return true to continue to read
        if ( const SharedBuffer *b = beg ) {
            // first buff
            const SharedBuffer *n = b->next;
...
public:
    SharedBuffer  *beg; ///< first item
    SharedBuffer  *end; ///< last item
